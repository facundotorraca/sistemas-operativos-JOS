...
### 1 - Primeras 5 lineas de info registers:
...
Su valor es 0x23 ya que fue seteado en env_alloc() con GD_UD | 3 (donde GD_UD es 0x20, representa el global descriptor user data y el 3 es para indicar el modo usuario (ring 3))


uint16_t tf_padding1 (16 bits mas altos del anterior);

uint16_t tf_ds    = 0x00000023;  ---  Direccion del data segment
Su valor es 0x23 ya que fue seteado en env_alloc() con GD_UD | 3 (GD_UD es 0x20).
...
Su valor es distinto de 0 ya que es el valor del entry point del elf (_start), que fue configurado en load_icode().


uint16_t tf_cs     = 0x0000001b; ---  Code segment
Su valor es 0x1b ya que fue seteado en env_alloc() con GD_UT | 3 (donde GD_UT es 0x18, representa el global descriptor user text y el 3 es para indicar el modo usuario (ring 3)).
...
Su valor es 0xeebfe000 ya que fue seteado en env_alloc() con USTACKTOP (definido en memlayout.h como 0xeebfe000)
...
Lo que sucede es que como todavia no esta implementado el handler de la interrupcion syscall 
(TRAPHANDLER_NOEC(trap_48, T_SYSCALL)) el sistema loopea en el booteo si se continua con c y se detiene cada vez en el 
breakpoint seteado previamente en env_pop_tf.
