...
sys_env_set_status(envid_t envid, int status)
{
	...
	struct Env *e;
	int r;
	if ((r = envid2env(envid, &e, /*checkperm:*/ 1)) < 0)
	    return r;
	e->env_status = status;
	return 0;
}
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
    ...
    if ((uintptr_t)tf >= UTOP)
        panic("Syscall set trapframe trapframe address is bigger than UTOP");
    if ((tf->tf_cs & 3) != 3)
        panic("Syscall set trapframe wrong CPL (ring level is not 3)");
    //Assign trapframe to environment
    e->env_tf = *tf;
    //Enable interrupts
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    //Page insert returns 0 on success or -E_NO_MEM on failure
